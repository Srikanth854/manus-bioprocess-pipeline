...
def build_dim_process(df):
    cols = [
        c
        for c in ["reactor_type", "rxt_volume", "media", "temp", "oxygen"]
        if c in df.columns
    ]
    dim = df[cols].drop_duplicates().reset_index(drop=True)
...
def chart_data_quality_dashboard(df_bronze, df_silver):
    ...
    ax.set_ylabel("Null %")
    ax.set_title(
        "Data Quality: Null % Before vs After Silver Cleaning\nBronze = raw landing  |  Silver = cleaned and imputed",
        fontweight="bold",
    )
    ax.legend()
...
def chart_pipeline_run_summary(df_bronze, df_silver, df_gold):
    ...
    ax2.set_title(
        "Column Count at Each Layer\n(Silver adds derived features and outlier flags)",
        fontweight="bold",
    )
    ...
    fig.suptitle(
        f"Manus Bioprocess Pipeline Run Summary  |  Pipeline v{PIPELINE_VERSION}  |  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
        fontsize=12,
        fontweight="bold",
    )
    plt.tight_layout()
...
def chart_business_aggregations(df):
    ...
    if "product_name" in df.columns and "titer" in df.columns:
        ...
        prod = (
            df2.groupby("product_name")["titer_num"]
        ...
        axes[0].barh(
            prod["product_name"], prod["avg_titer"], color=MANUS_COLORS[1], alpha=0.85
        )
    ...
    if "reactor_type" in df.columns and "yield" in df.columns:
        ...
        reac = (
            df3.groupby("reactor_type")["yield_num"]
    ...
    if "temp" in df.columns:
        ...
        for i, val in enumerate(tc.values):
            axes[2].text(
                i, val + 0.3, str(val), ha="center", fontweight="bold", fontsize=9
            )
...
def chart_data_lineage(df_bronze, df_silver, df_gold):
    ...
    ax.axis("off")
    fig.suptitle(
        "Manus Bioprocess Pipeline — Data Lineage Report",
        fontsize=15,
        fontweight="bold",
        y=0.98,
    )
    ...
    draw_box(
        ...
        [
            ...
            f"Rows: {len(df_silver):,}",
            f"Cols: {len(df_silver.columns)}",
            "Nulls imputed",
            "Outliers flagged",
            "Features derived",
        ],
    ...
    for label, color, x, checks in [
        (
            ...
            "#E67E22",
            3.8,
            ["Row count >= 10", "Metadata present", "Null % checked"],
        ),
        (
            "Silver QC",
            "#2980B9",
            7.6,
            ["Nulls imputed", "Temp in range", "Outliers flagged"],
        ),
        (
            "Gold QC",
            "#27AE60",
            11.4,
            ["Fact table exists", "Dim tables exist", "run_key unique"],
        ),
    ]:
        rect = mpatches.FancyBboxPatch(
            (x, 1.5),
