...
def upsert_gold(record: dict):
    ...
    try:
        ...
        """, (record["batch_id"], record["tank_id"])).fetchone()
...
def log_pipeline_run(run_id: str, layer: str, rows: int,
...
def get_layer_counts():
    ...
    try:
        ...
        gold   = conn.execute("SELECT COUNT(*) FROM gold_fermentation_summary").fetchone()[0]
