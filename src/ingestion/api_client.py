    ...
    for attempt in range(1, API_RETRY_LIMIT + 1):
        try:
            ...
            if response.status_code == 200:
                ...
                log.warning(f"PubChem returned {response.status_code} for {compound_name} (attempt {attempt})")

        except requests.exceptions.Timeout:
            log.warning(f"PubChem request timed out for {compound_name} (attempt {attempt})")
        except requests.exceptions.RequestException as e:
            log.warning(f"PubChem request error for {compound_name}: {e} (attempt {attempt})")
...
def enrich_products(df: pd.DataFrame, product_col: str = "product_name") -> pd.DataFrame:
    """
    ...
    log.info(f"Fetching PubChem data for {len(unique_products)} unique products: {unique_products}")
